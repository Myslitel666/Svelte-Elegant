<script lang="ts">
	...
	themeStore.subscribe((value) => {
		...
		if (!isBackgroundColorFromUser)
			backgroundColor =
				variant === 'Filled' ? theme.surface.filled.background : theme.palette.background;
		if (!isBorderColorFromUser)
			borderColor = variant === 'Filled' ? theme.border.active.color : theme.border.focused.color;
		...
		if (!isBackgroundColorFromUser)
			fill = variant === 'Filled' ? theme.surface.solid.background : theme.palette.background;
	});
